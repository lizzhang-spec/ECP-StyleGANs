...
from model import StyledConvBlock
...
def zero_out_conv_layer_channels(layer, prune_ratio=0.5):
    """
    Zero out channels in a convolutional layer uniformly.
    This ensures equal spacing between pruned channels instead of random selection.
    ...
    num_channels_to_prune = int(in_channels * prune_ratio)

    if num_channels_to_prune == 0:
        return layer 

    prune_channels = np.linspace(0, in_channels - 1, num_channels_to_prune, dtype=int)

    print(f"Before pruning: {layer.__class__.__name__} - Input channels: {in_channels}, Output channels: {layer.out_channels}")
    ...
    with torch.no_grad():
        ...
        if layer.bias is not None:
            ...
            print("bias Zero out done")

    print(f"After pruning: {layer.__class__.__name__} - Input channels: {in_channels}, "f"Zeroed-out channels: {num_channels_to_prune}")
...
def apply_zero_out_pruning_to_model(model, prune_ratio=0.5):
	"""
	Apply random zeroing out of channels to all convolutional layers in the model.

	:param model: The model to prune.
	:param prune_ratio: The percentage of channels to prune (0 to 1).
	:return: The model with zeroed-out channels.
	"""

	for name, module in model.named_modules():
		if isinstance(module, StyledConvBlock):
			module.conv2.conv = zero_out_conv_layer_channels(module.conv2.conv, prune_ratio) 
...
if __name__ == '__main__':
	...
	if not args.supervised and not args.init_D:
		D_target.module.load_state_dict(new_state_dict)
