...
class Generator(nn.Module):
    ...
    def forward(self, style, noise, step=0, alpha=-1, mixing_range = (-1, -1)):
        out = noise[0]
        # print(f"Step: {step}, Styles: {len(style)}, {[s.shape for s in style]}, Inject Index: {len(style) - 1}, Noise: {len(noise)}, {[n.shape for n in noise]}")
        ...
        for i, (conv, to_rgb) in enumerate(zip(self.progression, self.to_rgb)):
            if mixing_range == (-1, -1):
                ...
                # print("style_step.shape = ", style_step.shape)

            else:
                # print("mixing_range is not (-1, -1)")
            ...
            if i == step:
                out = to_rgb(out)
                # print("to_rgb_out.shape = ", out.shape)
    ...
    def forward(self, input, step=0, alpha=-1, get_feature=False, feature_loc=0, linearize=True):
        for i in range(step, -1, -1):
            ...
            if i == 0:
                ...
                # Resize mean_std to match out's spatial dimensions
                mean_std = F.interpolate(mean_std, size=(out.size(2), out.size(3)), mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    gen_i, gen_j = (10, 5)
    fixed_noise1 = torch.randn(gen_i, gen_j, 512)
    # print("fixed_noise1.shape = ", fixed_noise1.shape)

    for i in range(gen_i):
        netG(fixed_noise1[i], step=7, alpha=1)
    ...
    # FLOPs
    # Using torchinfo.summary to compute FLOPs
    summary(netG, input_data=tuple(fixed_noise1), forward_kwargs={"step": step, "alpha": alpha}, col_names=["output_size", "num_params", "mult_adds"])
    def wrapper_model(input_data):
        return netG(input_data, step=step, alpha=alpha)

    # Example usage
    summary(wrapper_model, input_data=tuple(fixed_noise1), col_names=["output_size", "num_params", "mult_adds"])
