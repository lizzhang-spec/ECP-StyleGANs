...
from model import Generator, PixelNorm, EqualLinear, StyledConvBlock
...
def finetune_supervised(args, dataset, G_target):
	...
	for i in pbar:
		...
		state_msg += ''.join([f' {key}: {val:.2f};' for (key, val) in metrics.items() if 'norm' not in key]) 
...
def zero_out_conv_layer_l1_norm(layer, prune_ratio=0.5):
    """
    Zero out channels in a convolutional layer based on the L1 norm of the weights.
    Prints the number of input channels before and after pruning.

    :param layer: The convolutional layer to prune.
    :param prune_ratio: The percentage of channels to prune (0 to 1).
    :return: The modified layer with zeroed channels.
    """
    if not hasattr(layer, 'weight_orig'): 
        print(f"Skipping layer {layer.__class__.__name__} as it has no 'weight'.")
        return layer

    weight = layer.weight_orig.data  

    l1_norms = torch.norm(weight, p=1, dim=(1, 2, 3))  
    num_channels_to_prune = int(weight.shape[0] * prune_ratio)

    if num_channels_to_prune == 0:
        print(f"{layer.__class__.__name__}: No pruning applied (prune ratio too low).")
        return layer  

    prune_channels = torch.argsort(l1_norms)[:num_channels_to_prune]  
    
    print(f"Before pruning: {layer.__class__.__name__} - Output channels: {weight.shape[0]}, "
          f"Pruning {num_channels_to_prune} channels")

    with torch.no_grad():
        layer.weight_orig[:, prune_channels, :, :] = 0 

        if layer.bias is not None:
            layer.bias[prune_channels] = 0

    print(f"After pruning: {layer.__class__.__name__} - Output channels: {weight.shape[0]}, "
          f"Zeroed-out channels: {num_channels_to_prune}")

    return layer 



def apply_zero_out_pruning_to_model(model, prune_ratio=0.5):
    """
    Apply L1-norm-based zeroing out of channels (weights + biases) in StyledConvBlock layers.
    Prints the number of channels before and after pruning.
    """
    for name, module in model.named_modules():
        if isinstance(module, StyledConvBlock):
            module.conv2.conv = zero_out_conv_layer_l1_norm(module.conv2.conv, prune_ratio)
...
if __name__ == '__main__':
	...
	G_target = apply_zero_out_pruning_to_model(G_target, prune_ratio)
	G_running_target = apply_zero_out_pruning_to_model(G_running_target, prune_ratio)
	G_source = apply_zero_out_pruning_to_model(G_source, prune_ratio)
...
# # Example usage:
# gen = StyledGenerator(code_dim=512, n_mlp=8, prune_ratio=0.5)  # Prune 50% of channels
# print(gen)


# conv = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3)
# pruned_conv = prune_conv_layer(conv, num_channels_to_keep=8)
# print(pruned_conv)
